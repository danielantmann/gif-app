...
import * as gifAction from "../actions/get-gifs-by-query";
...
describe("useGifs", () => {
  ...
  test("should return a list og gifs when handleTermClicked is called", async () => {
    const { result } = renderHook(() => useGifs());
    await act(async () => {
  ...
  test("shoulf not retun no more than 8 previous terms", async () => {
    const { result } = renderHook(() => useGifs());
